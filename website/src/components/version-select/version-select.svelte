<script lang="ts">
	...
	type Props = {
		...
		version: string;
		// null when the live site's manifest could not be read
	...
	const versionListed = $derived(
		manifest !== null && manifest.versions.some((entry) => entry.version === version),
	);

	function switchVersion(event: Event): void {
		const chosen = (event.currentTarget as HTMLSelectElement).value;
		if (manifest === null || chosen === version) {
			return;
		}
