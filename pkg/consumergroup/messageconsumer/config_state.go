...
type configState struct {
	...
	declared *MessageConsumerMetadata
}

func newConfigState(cfg *MessageConsumerConfig, declared *MessageConsumerMetadata) (*configState, error) {
	if cfg == nil {
...
func (s *configState) current() *MessageConsumerConfig {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.cfg
}
...
func (s *configState) replace(declared *MessageConsumerMetadata) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
