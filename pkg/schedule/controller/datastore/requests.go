...
// ListRequests is the schedule's newest limit requests, one row per
// (request, consumer group that receives it), newest request first.
func (d *ScheduleDatastore) ListRequests(ctx context.Context, jobRequestsTopicId int64, scheduleId int64, name string, limit int) ([]JobRequestStatusData, error) {
	var requests []JobRequestStatusData
	err := d.DatastoreRetry.Wrap(ctx, func() error {
		var err error
		requests, err = d.listRequests(ctx, jobRequestsTopicId, scheduleId, name, limit)
		return err
	})
	return requests, err
}
...
// jobMessages is the newest limit message-log rows on the schedule's message
// key, newest first.
func (d *ScheduleDatastore) jobMessages(ctx context.Context, jobRequestsTopicId int64, messageKey string, limit int) ([]jobMessageData, error) {
	sql := fmt.Sprintf(`
		-- vulkan: schedule.jobMessages
