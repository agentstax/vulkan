...
func (c *ExceptionConsumerController) KillExceptions(ctx context.Context, topicId int64, groupId int64, maxRetries int, disableDeliveryLog bool) error {
	if topicId <= 0 {
	...
	return c.datastore.KillExceptions(ctx, topicId, groupId, maxRetries, disableDeliveryLog)
}
...
func (c *ExceptionConsumerController) ClaimExceptions(ctx context.Context, topicId int64, groupId int64, limit int, maxRetries int, leaseDuration time.Duration, disableDeliveryLog bool) ([]ClaimedException, error) {
	if topicId <= 0 {
	...
	claimed, err := c.datastore.ClaimExceptions(ctx, topicId, groupId, limit, maxRetries, leaseDuration, disableDeliveryLog)
	if err != nil {
		return nil, err
	}
...
func (c *ExceptionConsumerController) RecordExceptionSuccess(ctx context.Context, exception *ClaimedException, keyClaim *keyleasecontroller.KeyLeaseClaim) error {
	if exception == nil {
		return errors.New("exception must not be nil")
	}

	return c.datastore.RecordExceptionSuccess(ctx, toExceptionData(exception), toKeyLeaseData(keyClaim))
}

// RecordExceptionFailure resets the row so it can be retried, or marks it
// 'dead' once retryPolicy's budget is spent.
// A non-nil keyClaim frees the key in the same transaction.
func (c *ExceptionConsumerController) RecordExceptionFailure(ctx context.Context, retryPolicy *retry.Policy, exception *ClaimedException, failureErr error, disableDeliveryLog bool, keyClaim *keyleasecontroller.KeyLeaseClaim) error {
	if retryPolicy == nil {
	...
	return c.datastore.RecordExceptionFailure(ctx, retryPolicy, toExceptionData(exception), failureErr, disableDeliveryLog, toKeyLeaseData(keyClaim))
}

// RecordExceptionTerminal marks the row 'dead' -- no retry could succeed.
// A non-nil keyClaim frees the key in the same transaction.
func (c *ExceptionConsumerController) RecordExceptionTerminal(ctx context.Context, exception *ClaimedException, failureErr error, disableDeliveryLog bool, keyClaim *keyleasecontroller.KeyLeaseClaim) error {
	if exception == nil {
		return errors.New("exception must not be nil")
	}
	if failureErr == nil {
	...
	return c.datastore.RecordExceptionTerminal(ctx, toExceptionData(exception), failureErr, disableDeliveryLog, toKeyLeaseData(keyClaim))
}
...
func (c *ExceptionConsumerController) RecordExceptionSuperseded(ctx context.Context, exception *ClaimedException, disableDeliveryLog bool) error {
	if exception == nil {
		return errors.New("exception must not be nil")
	}

	return c.datastore.RecordExceptionSuperseded(ctx, toExceptionData(exception), disableDeliveryLog)
}
