...
func newExceptionRunner[Message any](base *consumerbase.BaseConsumer[Message], consumers *controller.ExceptionConsumerController, cfg *ExceptionConsumerConfig) (*exceptionRunner[Message], error) {
	if base == nil {
...
func (r *exceptionRunner[Message]) exceptionClaim(ctx context.Context) error {
	...
	if err := r.consumers.KillExceptions(ctx, r.Topic.Id, r.Owner.ConsumerGroupId, r.cfg.MessageMax.Retry.MaxRetries, r.Topic.DisableDeliveryLog); err != nil {
		return err
	}

	claimed, err := r.consumers.ClaimExceptions(ctx, r.Topic.Id, r.Owner.ConsumerGroupId, r.cfg.BatchLimit, r.cfg.MessageMax.Retry.MaxRetries, leaseDuration, r.Topic.DisableDeliveryLog)
	if err != nil {
		return err
	}
...
func (r *exceptionRunner[Message]) processException(ctx context.Context, exception *controller.ClaimedException) error {
	...
	runCtx := message.WithMeta(ctx, toExceptionMessageMeta(exception, resolvedOptions))
...
func (r *exceptionRunner[Message]) recordSuccess(ctx context.Context, exception *controller.ClaimedException, keyClaim *keyleasecontroller.KeyLeaseClaim) error {
	...
	err := r.consumers.RecordExceptionSuccess(recordCtx, exception, keyClaim)
...
func (r *exceptionRunner[Message]) recordFailure(ctx context.Context, exception *controller.ClaimedException, resolvedOptions *common.MessageOptions, runErr error, keyClaim *keyleasecontroller.KeyLeaseClaim) error {
	...
	err := r.consumers.RecordExceptionFailure(recordCtx, resolvedOptions.Retry, exception, runErr, r.Topic.DisableDeliveryLog, keyClaim)
	return r.absorbLostLease(ctx, exception, err)
}
...
func (r *exceptionRunner[Message]) recordTerminal(ctx context.Context, exception *controller.ClaimedException, runErr error, keyClaim *keyleasecontroller.KeyLeaseClaim) error {
	...
	err := r.consumers.RecordExceptionTerminal(recordCtx, exception, runErr, r.Topic.DisableDeliveryLog, keyClaim)
	return r.absorbLostLease(ctx, exception, err)
}

func (r *exceptionRunner[Message]) recordSuperseded(ctx context.Context, exception *controller.ClaimedException) error {
	err := r.consumers.RecordExceptionSuperseded(ctx, exception, r.Topic.DisableDeliveryLog)
	return r.absorbLostLease(ctx, exception, err)
}
