...
func (d *ExceptionConsumerDefinition[Message]) Declare(ctx context.Context, owner *common.Owner) error {
	return d.DeclareWorker(ctx, owner, toExceptionConsumerMetadata(d.Config))
}

// a nil Execution is a declined claim, not an error -- try again later.
func (d *ExceptionConsumerDefinition[Message]) Provision(ctx context.Context, workerId int64, owner *common.Owner, metadata any) (worker.Execution, error) {
