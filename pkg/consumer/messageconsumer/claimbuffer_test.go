...
func testToken(b byte) uuid.UUID {
	return uuid.UUID([16]byte{0: b})
}

func testRange(token uuid.UUID, ids ...int64) *consumercontroller.ClaimedRange {
	messages := make([]consumercontroller.Message, len(ids))
	for i, id := range ids {
		messages[i] = consumercontroller.Message{Id: id}
	}
	low := int64(0)
	if len(ids) > 0 {
		low = ids[0] - 1
	}
	return &consumercontroller.ClaimedRange{
		Lease:    consumercontroller.RangeLease{Token: token, Low: low, High: low + int64(len(ids))},
		Messages: messages,
	}
}
...
func TestClaimBuffer_ResolveResultExclusivity(t *testing.T) {
	...
	if got := countKind(commit.Outcomes, consumercontroller.OutcomeException); got != wantExceptions {
		t.Fatalf("exceptions = %d, want %d", got, wantExceptions)
	}
	if got := countKind(commit.Outcomes, consumercontroller.OutcomeTerminal); got != wantTerminals {
		t.Fatalf("terminals = %d, want %d", got, wantTerminals)
	}
}
...
func TestClaimBuffer_RemoveAllExactlyOnce(t *testing.T) {
	...
	tokens := []uuid.UUID{testToken(10), testToken(11), testToken(12)}
...
func TestRangeState_ContiguousResolvedStopsAtGap(t *testing.T) {
	...
	if len(outcomes) != 0 {
		t.Fatalf("outcomes past the gap must be excluded, got %d", len(outcomes))
	}
}
...
func TestRangeState_ContiguousResolvedNoGap(t *testing.T) {
	...
	exceptions := countKind(outcomes, consumercontroller.OutcomeException)
	terminals := countKind(outcomes, consumercontroller.OutcomeTerminal)
	if exceptions != 1 || terminals != 1 {
		t.Fatalf("exceptions=%d terminals=%d, want 1 and 1", exceptions, terminals)
	}
}
...
func TestRangeState_ContiguousResolvedNothingDone(t *testing.T) {
	...
	if outcomes != nil {
		t.Fatal("expected no outcomes when nothing resolved")
	}
}
...
func TestRangeState_SupersededAndDeferredCollected(t *testing.T) {
	...
	if countKind(outcomes, consumercontroller.OutcomeException) != 0 || countKind(outcomes, consumercontroller.OutcomeTerminal) != 0 {
		t.Fatalf("outcomes = %+v, want no exceptions or terminals", outcomes)
	}
	superseded := outcomesOfKind(outcomes, consumercontroller.OutcomeSuperseded)
	if len(superseded) != 1 || superseded[0].MessageId != 401 {
		t.Fatalf("superseded = %+v, want exactly message 401", superseded)
	}
	deferred := outcomesOfKind(outcomes, consumercontroller.OutcomeDeferred)
...
func countKind(outcomes []consumercontroller.MessageOutcome, kind consumercontroller.OutcomeKind) int {
	count := 0
...
func outcomesOfKind(outcomes []consumercontroller.MessageOutcome, kind consumercontroller.OutcomeKind) []consumercontroller.MessageOutcome {
	var matched []consumercontroller.MessageOutcome
	for _, outcome := range outcomes {
		if outcome.Kind == kind {
