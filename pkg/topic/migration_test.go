...
func TestValidateRegistry(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ms := make([]TopicMigration, len(tt.versions))
			for i, v := range tt.versions {
				ms[i] = TopicMigration{Version: v}
			}
...
func TestTopicMigrationsRegistryValid(t *testing.T) {
	if err := validateRegistry(topicMigrations); err != nil {
		t.Fatalf("topicMigrations invalid: %v", err)
	}
}
