...
func (d *MetricsCollectorDefinition) Declare(ctx context.Context, owner *common.Owner) error {
	if err := controller.ValidateOwner(owner, common.OwnerSystem, WorkerMetricsCollector); err != nil {
		return err
	}

	return d.workers.RegisterWorker(ctx, WorkerMetricsCollector, owner, &controller.WorkerConfig{
...
func (d *MetricsCollectorDefinition) Provision(ctx context.Context, workerId int64, owner *common.Owner, metadata any) (worker.Execution, error) {
	...
	producerInstance, err := d.producer.Register(ctx, metrics.TopicName, topic.SchemaVersion(1))
	if err != nil {
		return nil, err
	}
	claimed, err := d.workers.RegisterInstance(ctx, workerId, owner, common.OwnerSystem, WorkerMetricsCollector, d.Config.InstanceTTL)
	if err != nil || claimed == nil {
		return nil, err
	}
	return newMetricsCollectorInstance(d, owner, claimed, parsed, producerInstance)
}
