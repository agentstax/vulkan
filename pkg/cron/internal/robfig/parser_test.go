...
func TestParseSchedule(t *testing.T) {
	...
	entries := []struct {
		...
		{secondParser, "0 5 * * * *", every5min(time.UTC)},
		{standardParser, "5 * * * *", every5min(time.UTC)},
		{secondParser, "CRON_TZ=UTC  0 5 * * * *", every5min(time.UTC)},
		{standardParser, "CRON_TZ=UTC  5 * * * *", every5min(time.UTC)},
		{secondParser, "CRON_TZ=Asia/Tokyo 0 5 * * * *", every5min(tokyo)},
		{secondParser, "@every 5m", ConstantDelaySchedule{5 * time.Minute}},
		{secondParser, "@midnight", midnight(time.UTC)},
		{secondParser, "TZ=UTC  @midnight", midnight(time.UTC)},
		{secondParser, "TZ=Asia/Tokyo @midnight", midnight(tokyo)},
		{secondParser, "@yearly", annual(time.UTC)},
		{secondParser, "@annually", annual(time.UTC)},
		{
			parser: secondParser,
			expr:   "* 5 * * * *",
...
func TestOptionalSecondSchedule(t *testing.T) {
	...
	entries := []struct {
		...
		{"0 5 * * * *", every5min(time.UTC)},
		{"5 5 * * * *", every5min5s(time.UTC)},
...
func TestStandardSpecSchedule(t *testing.T) {
	entries := []struct {
		...
		{
			expr:     "5 * * * *",
			expected: &SpecSchedule{1 << seconds.min, 1 << 5, all(hours), all(dom), all(months), all(dow), time.UTC},
		},
		{
			expr:     "@every 5m",
