...
func (d *JanitorProvisioner) Provision(ctx context.Context, declared *worker.WorkerData) (worker.Execution, error) {
	...
	claimed, err := d.workers.RegisterInstance(ctx, declared.Id, declared.Owner, common.OwnerSystem, WorkerConsumerGroupJanitor, d.Config.InstanceTTL)
	if err != nil || claimed == nil {
		return nil, err
	}
	return newJanitorInstance(d, declared.Owner, claimed, parsed)
}
