...
// a bug.
func newCompactionReadCostAlert(owner *common.Owner, count int64, threshold int64) (*alert.Alert, error) {
	if threshold <= 0 {
	...
	return alert.NewAlert(AlertCompactionReadCost, owner, alert.StatusActive, alert.SeverityWarn, message, &alert.AlertOptions{
		Detail: detail,
		Hint:   hint,
		Data:   data,
	})
