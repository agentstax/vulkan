...
import (
	...
	"github.com/agentstax/vulkan/pkg/alert/compactionreadcost/datastore"
...
func NewHandler(ds *coredatastore.PostgresDatastore, publisher *alert.Publisher, cfg *HandlerConfig) (*Handler, error) {
	...
	alertDatastore, err := datastore.NewCompactionReadCostDatastore(ds, &datastore.CompactionReadCostDatastoreConfig{
		Logger: cfg.Logger,
		Retry:  cfg.Retry,
	})
	if err != nil {
...
func (h *Handler) Handle(ctx context.Context, request *cron.JobRequest) error {
	...
	if jobData.Threshold == 0 {
		jobData.Threshold = warnPartitions
	}
	...
	for _, topic := range topics {
		...
		found, err := h.evaluate(ctx, owner, jobData.Threshold)
		if err != nil {
			errs = errors.Join(errs, err)
			continue
		}
...
func (h *Handler) evaluate(ctx context.Context, owner *common.Owner, threshold int64) (*alert.Alert, error) {
	compacted, err := h.datastore.Compacted(ctx, owner.TopicId)
	if err != nil {
		return nil, err
	}
