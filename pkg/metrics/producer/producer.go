...
func (p *MetricsProducer) Add(topicId int64, group string, messageId int64, attempt int) {
	p.enqueue(metrics.NewGoRoutineEvent(metrics.EventAbandoned, topicId, group, messageId, attempt, time.Now()))
}

func (p *MetricsProducer) Remove(topicId int64, group string, messageId int64, attempt int) {
