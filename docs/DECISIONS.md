...
- 0565 2026-08-20 accepted (pre-v1) — pkg/common/logging grows a private record pipeline under the unchanged Logger seam (slog Logger/Handler/Record split rebuilt: record struct + one-method handler; stages captureHandler/suppressHandler/drainHandler/enrichHandler/sinkHandler, one assembly constructor, fixed order capture→suppress→drain→enrich→sink); bufferLogger/withLogger/LoggerWith hoisting+idempotence guards dissolve into stages; narrows 0559 (wrapper mechanism replaced, boundaries/ring/drain semantics stand); research: slog handlers, zap sampler core, Logback turbo filters, K8s EventCorrelator — no system composes concerns on a leveled front-end
