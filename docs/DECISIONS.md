...
- 0577 2026-08-22 accepted (pre-v1) — worker metadata history = append-only worker_log (completes [0570] reservation): full snapshot (name denormalized for join-free operator scans, metadata, target_instances, declared_by=ProcessIdentity, declared_at), both writes inside registerWorker's existing tx (create path + replace path only when metadata changed — no-change redeclares write nothing, log tracks change never restarts), machinery never reads it, no retention (worker_log/topic_log TTL revisit parked); no-change UPDATE dead tuple left out of scope
