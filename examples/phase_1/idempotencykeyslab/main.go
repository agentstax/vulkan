...
//
// Every publish is always protected -- there is no opt-out.
//
// Five scenarios:
...
//     double-publishing. Also confirms the landed row's own idempotency_key
//     column (a durable copy that outlives the claim-gate table's own TTL
//     sweep) matches the key that produced it.
...
func main() {
	...
	fmt.Println("   a retried AppendMessage under the same key lands exactly once, and the")
	fmt.Println("   landed row's own idempotency_key column matches it; distinct keys never")
	fmt.Println("   collide; an unset key protects only within one call; the sweep drains")
	fmt.Println("   expired claims without touching live ones; and IdempotencyKeyTTL survives")
	fmt.Println("   a topic re-registration unchanged.")
}
...
func duplicateKeyScenario(ctx context.Context, ds *coredatastore.PostgresDatastore) {
	...
	assertMessageLogIdempotencyKey(ctx, ds, tp.Id, key)
	fmt.Println("  ✓ the landed row's own idempotency_key column matches the key that produced it")
}
...
func assertMessageLogIdempotencyKey(ctx context.Context, ds *coredatastore.PostgresDatastore, topicID int64, want uuid.UUID) {
	var got uuid.UUID
	must(ds.Pool.QueryRow(ctx, fmt.Sprintf(`SELECT idempotency_key FROM message_log_%d;`, topicID)).Scan(&got))
	if got != want {
		die(fmt.Sprintf("message_log_%d.idempotency_key = %s, want %s", topicID, got, want))
	}
}
