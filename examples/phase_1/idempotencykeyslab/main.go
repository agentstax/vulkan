...
func duplicateKeyScenario(ctx context.Context, ds *iDatastore.PostgresDatastore) {
...
func distinctKeysScenario(ctx context.Context, ds *iDatastore.PostgresDatastore) {
...
func autoGeneratedKeyScenario(ctx context.Context, ds *iDatastore.PostgresDatastore) {
...
func sweepScenario(ctx context.Context, ds *iDatastore.PostgresDatastore) {
...
func configRoundTripScenario(ctx context.Context, ds *iDatastore.PostgresDatastore) {
	...
	tp1, err := mAdmin.RegisterTopic(ctx, topicName, &topiccontroller.TopicConfig{PartitionSize: 1000})
	must(err)
	...
	tp2, err := mAdmin.RegisterTopic(ctx, topicName, &topiccontroller.TopicConfig{PartitionSize: 1000})
	must(err)
	...
	tp3, err := mAdmin.RegisterTopic(ctx, topicName, &topiccontroller.TopicConfig{PartitionSize: 1000, IdempotencyKeyTTL: 2 * time.Hour})
	must(err)
	...
	_, err = mAdmin.RegisterTopic(ctx, topicName, &topiccontroller.TopicConfig{PartitionSize: 2000, IdempotencyKeyTTL: 2 * time.Hour})
