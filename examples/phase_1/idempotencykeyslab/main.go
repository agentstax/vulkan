...
func autoGeneratedKeyScenario(ctx context.Context, pool *pgxpool.Pool) {
	...
	// one claim row per call is the proof a fresh key was minted each time --
	// a reused key would have collapsed them into one
	assertIdempotencyKeysCount(ctx, ds, tp.Id, 3)
	assertMessageLogCount(ctx, ds, tp.Id, 3)
	fmt.Println("  ✓ 3 separate calls with no explicit key -> 3 distinct keys, 3 distinct rows")
}
...
func configRoundTripScenario(ctx context.Context, pool *pgxpool.Pool) {
	...
	tp1, err := client.Topic(topicName).Register(ctx, &vulkan.TopicConfig{PartitionSize: 1000})
	must(err)
	...
	tp2, err := client.Topic(topicName).Register(ctx, &vulkan.TopicConfig{PartitionSize: 1000})
	must(err)
	...
	tp3, err := client.Topic(topicName).Register(ctx, &vulkan.TopicConfig{PartitionSize: 1000, IdempotencyKeyTTL: 2 * time.Hour})
	must(err)
	...
	_, err = client.Topic(topicName).Register(ctx, &vulkan.TopicConfig{PartitionSize: 2000, IdempotencyKeyTTL: 2 * time.Hour})
