...
func duplicateKeyScenario(ctx context.Context, ds *coredatastore.PostgresDatastore) {
	...
	for range 3 {
		_, err := wp.ProduceFunc(ctx, fn, opts)
		must(err)
	}
...
func autoGeneratedKeyScenario(ctx context.Context, ds *coredatastore.PostgresDatastore) {
	...
	for range 3 {
		_, err := wp.ProduceFunc(ctx, func(ctx context.Context, tx producer.Tx, k uuid.UUID) (*common.Work, error) {
			if k == uuid.Nil {
...
func sweepScenario(ctx context.Context, ds *coredatastore.PostgresDatastore) {
	...
	for range 5 {
		_, err := wp.ProduceFunc(ctx, fn, producer.ProduceOptions{})
		must(err)
	}
...
func skipIdempotencyScenario(ctx context.Context, ds *coredatastore.PostgresDatastore) {
	...
	for range 3 {
		_, err := wp.ProduceFunc(ctx, fn, producer.ProduceOptions{IdempotencyKey: key, SkipIdempotency: true})
		must(err)
	}
	...
	for range 3 {
		_, err := wp.ProduceFunc(ctx, protectedFn, producer.ProduceOptions{IdempotencyKey: protectedKey})
		must(err)
	}
