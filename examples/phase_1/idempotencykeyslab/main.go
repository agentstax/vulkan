...
func duplicateKeyScenario(ctx context.Context, ds *iDatastore.PostgresDatastore) {
	...
	calls := 0
	fn := func(ctx context.Context, tx vulkan.Tx, gotKey string) (*common.Work, error) {
		calls++
		if gotKey != key {
...
func autoGeneratedKeyScenario(ctx context.Context, ds *iDatastore.PostgresDatastore) {
	...
	for range 3 {
		_, err := wpInstance.ProduceFunc(ctx, func(ctx context.Context, tx vulkan.Tx, k string) (*common.Work, error) {
			if k == "" {
...
func sweepScenario(ctx context.Context, ds *iDatastore.PostgresDatastore) {
	...
	for range 5 {
		_, err := wpInstance.ProduceFunc(ctx, fn, vulkan.ProduceOptions{})
		must(err)
	}
...
func configRoundTripScenario(ctx context.Context, ds *iDatastore.PostgresDatastore) {
	...
	tp1, err := client.RegisterTopic(ctx, topicName, &vulkan.TopicConfig{PartitionSize: 1000})
	must(err)
	...
	tp2, err := client.RegisterTopic(ctx, topicName, &vulkan.TopicConfig{PartitionSize: 1000})
	must(err)
	...
	tp3, err := client.RegisterTopic(ctx, topicName, &vulkan.TopicConfig{PartitionSize: 1000, IdempotencyKeyTTL: 2 * time.Hour})
	must(err)
	...
	_, err = client.RegisterTopic(ctx, topicName, &vulkan.TopicConfig{PartitionSize: 2000, IdempotencyKeyTTL: 2 * time.Hour})
