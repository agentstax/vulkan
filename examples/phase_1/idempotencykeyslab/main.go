...
func configRoundTripScenario(ctx context.Context, ds *coredatastore.PostgresDatastore) {
	...
	tp1, err := mAdmin.RegisterTopic(ctx, topicName, &topic.Config{PartitionSize: 1000})
	must(err)
	...
	_, err = mAdmin.RegisterTopic(ctx, topicName, &topic.Config{PartitionSize: 1000})
	must(err)
	...
	_, err = mAdmin.RegisterTopic(ctx, topicName, &topic.Config{PartitionSize: 1000, IdempotencyKeyTTL: 2 * time.Hour})
	if err == nil {
